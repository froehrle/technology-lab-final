...
const FarmGrid = () => {
  ...
  return <div className="relative w-full max-w-4xl mx-auto">
      ...
      {/* Next Item Info */}
      {nextItem && <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm rounded-lg px-3 py-2 shadow-lg z-20">
      ...
      <div className="relative mx-auto rounded-3xl overflow-hidden shadow-2xl border-4 border-green-500/60" style={{
      width: '800px',
      height: '600px',
      ...
      backgroundPosition: 'center'
        ...
        gap: '8px'
          ...
          {gridLayout.map((row, rowIndex) => row.map((slot, colIndex) => <FarmSlot key={`${rowIndex}-${colIndex}`} slot={slot} isEmpty={!slot} />))}
        </div>
      </div>
    </div>;
};
